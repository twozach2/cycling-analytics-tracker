...
export const METHOD_DEFINITIONS: readonly MethodDefinition[] = [
  ...
  { id: "05", title: "Fitness and fatigue load", formula: "42-day EWMA fitness + 7-day EWMA fatigue", note: "Both estimates come from one daily training-load series. Fatigue / fitness is withheld until at least 42 days and 8 riding days are observed. These are workload models, not direct physiological measurements or injury predictions." },
  ...
  { id: "20", title: "Intention-aware ride reflection", formula: "saved idea revision + local ride date + environment + route evidence + recorded duration/load/intensity", note: "Automatic linking requires compatible date and indoor/outdoor setting, and route evidence raises confidence. Ambiguous rides are never guessed. Reflections describe what stayed close and what varied without grading optional focus blocks; every ride-driven adaptation and limitation is persisted locally." },
];
...
export function buildCyclingMarkdown(
  ...
  const lines = [
    ...
    ...(config.coachReport ? [
      ...
      `- Seven-day total load: ${config.coachReport.evidenceSummary.sevenDayLoad}`,
      `- Modeled 42-day fitness load: ${config.coachReport.evidenceSummary.fitnessLoad?.toFixed(1) ?? "Not available"}`,
      `- Modeled 7-day fatigue load: ${config.coachReport.evidenceSummary.fatigueLoad?.toFixed(1) ?? "Not available"}`,
      `- Modeled form (fitness − fatigue): ${config.coachReport.evidenceSummary.form?.toFixed(1) ?? "Not available"}`,
      `- Fatigue / fitness comparison: ${config.coachReport.evidenceSummary.loadRatio === null ? "Withheld" : `${config.coachReport.evidenceSummary.loadRatio.toFixed(2)}x`}`,
      `- Load-model evidence: ${config.coachReport.evidenceSummary.loadStatus} · ${config.coachReport.evidenceSummary.loadHistoryDays} days observed`,
