...
export function buildCyclingMarkdown(
  ...
  const lines = [
    ...
    ...(config.coachReport ? [
      ...
      `- Pre-ride plan (inferred): ${config.coachReport.completion.plannedMode}`,
      `- Completed-ride match: ${config.coachReport.completion.status}`,
      `- Completed stimulus: ${config.coachReport.completion.actualMode ?? "No ride detected"}`,
      `- Completion detail: ${config.coachReport.completion.detail}`,
